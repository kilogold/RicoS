...
- **Solana Pay** — `HELIUS_USDC_MINT`, `HELIUS_MERCHANT_RECIPIENT`, `HELIUS_API_KEY`, and `HELIUS_SOLANA_CLUSTER` (must match `NEXT_PUBLIC_HELIUS_SOLANA_CLUSTER` for checkout UI)
...
- `ADMIN_SESSION_SIGNING_SECRET` (signs the admin session cookie issued after a passkey ceremony)
...
- **Do not** merge `RicoS-Menu/preview` into `RicoS-Menu/main` to “promote” menu content. App repo `preview` → `main` merges are code-only. Promote a tested catalog with `bun scripts/promote-menu.mjs` in RicoS-Menu, which rewrites `catalogVersion`/`publishedAt` to satisfy the CI guard.
