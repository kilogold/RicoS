...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addItem = useCallback((id: string, selections: LineSelections = {}) => {
    ...
    setLines((prev) => {
      const existing = prev.find((l) => l.id === id && selectionSignature(l.selections) === sig);
      if (existing) {
        return prev.map((l) =>
  ...
  const removeItem = useCallback((id: string, selections: LineSelections = {}) => {
    ...
    setLines((prev) => prev.filter((l) => !(l.id === id && selectionSignature(l.selections) === sig)));
  }, []);

  const setQuantity = useCallback((id: string, selections: LineSelections, quantity: number) => {
    const normalized = normalizeSelections(selections);
    const sig = selectionSignature(normalized);
    if (quantity < 1) {
      setLines((prev) => prev.filter((l) => !(l.id === id && selectionSignature(l.selections) === sig)));
    ...
    setLines((prev) => {
      const has = prev.some((l) => l.id === id && selectionSignature(l.selections) === sig);
      if (!has) return [...prev, { id, quantity, selections: normalized }];
      return prev.map((l) =>
        l.id === id && selectionSignature(l.selections) === sig ? { ...l, quantity } : l,
      );
    });
  }, []);
  ...
  const value = useMemo(() => ({ lines, addItem, removeItem, setQuantity, clear }), [
    lines,
    addItem,
    removeItem,
    setQuantity,
