...
/** Poll raw.githubusercontent.com until the expected catalog version is visible. */
export async function waitForPublishedMenuCatalogVersion(expectedVersion: number): Promise<void> {
  for (let attempt = 1; attempt <= RAW_MENU_VERIFY_MAX_ATTEMPTS; attempt++) {
    try {
      const parsed = await fetchRemoteMenuCatalog();
      if (parsed.catalogVersion === expectedVersion) return;
    } catch {
  ...
  throw new Error(
    `Menu catalog v${expectedVersion} did not appear at MENU_PUBLISH_MENU_JSON_URL after ${RAW_MENU_VERIFY_MAX_ATTEMPTS} attempts.`,
  );
}
