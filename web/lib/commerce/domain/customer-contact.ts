...
/** Max characters in formatted US display string, e.g. `(787) 555-1234` (14 chars; not digit count). */
export const CUSTOMER_PHONE_FORMATTED_MAX_LEN = 14;
...
export function validateCustomerContact(input: CustomerContactInput): ValidateCustomerContactResult {
  ...
  if (customerPhone.length > CUSTOMER_PHONE_FORMATTED_MAX_LEN) {
    return {
      ok: false,
      error: `customerPhone must be at most ${CUSTOMER_PHONE_FORMATTED_MAX_LEN} characters`,
    };
  }
